...
class Sync():
	def __init__(self, comms):
		...
		self._overlap_tuning = 10.0		# increase to increase the amount of time gliders spend on the surface communicating with one another; 
	...
	def wait_until_ring_topology(self, k, dt, agent_id, state_machine, comms_list):
		if state_machine._on_surface_params["surface_time_ctr"] == 0:
			...
			self._dive_interval = self._dive_interval + self._overlap_tuning
			...
			print("INIT agent: {0}, order in queue: {1}, surface_time_threshold: {2}".format(agent_id, self._order_in_dive_queue, state_machine._on_surface_params["surface_time_threshold"]))
		...
		if ( (self.comms._on_surface - self._prev_on_surface).any() ):
			...
			if dived_idx_.sum() > 0:  									# if agent(s) dived
				self._order_in_dive_queue = self._order_in_dive_queue - 1
				state_machine._on_surface_params["surface_time_ctr"] = 0
				state_machine._on_surface_params["surface_time_threshold"] =  self._order_in_dive_queue*self._dive_interval
				print("DIVED UPDATE agent: {0}, order in queue: {1}, surface_time_threshold: {2}".format(agent_id, self._order_in_dive_queue, state_machine._on_surface_params["surface_time_threshold"]))
